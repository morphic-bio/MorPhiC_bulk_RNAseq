...
format_current_dataset_label <- function(x){
  
  x = gsub("_", " ", x)
  x = gsub("Pancreatic Islet day 18 ", "", x)
  x = gsub("WT KO ISL1 OE", "WT ISL1 OE", x)
  x = gsub("BFP OE", "control", x)
  x = gsub(" ISL1 OE ISL1 OE ", " ISL1 OE ", x)
  x = gsub(" ISL1 KO vs WT ISL1 KO ", " ISL1 KO vs WT ", x)
  x = gsub(" PAX6 KO vs WT PAX6 KO ", " PAX6 KO vs WT ", x)
  x = gsub(" PDX1 KO vs WT PDX1 KO ", " PDX1 KO vs WT ", x)
  x = gsub("up", "up-regulated", x)
  x = gsub("down", "down-regulated", x)
  
  substrings = str_split(x, " ")[[1]]
  x = paste0(paste(substrings[1:(length(substrings)-1)], collapse = " "), 
...
for(k in 1:length(df_list)){
  ...
  if (release_name == "2026_01_MSK_bulkRNAseq_ISL1_OE"){
    nm1 = format_current_dataset_label(nm1)
  }else{
    nm1 = gsub("_", " ", nm1)
    nm1 = gsub("PTC ", "", nm1)
  ...
  if (!grepl("Hypoxia vs Normoxia", nm1)){
    ...
    saved_figure_name = paste0(gsub(" ", "_", saved_figure_name), ".png")
  
    ggsave(filename=file.path(goseq_figure_dir, saved_figure_name),
           plot=canvas2,
