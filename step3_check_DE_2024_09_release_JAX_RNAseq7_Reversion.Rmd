---
title: "Check the results of differential expression analysis for 2024-09 release JAX_RNAseq7_Reversion"
...
extract_cores <- function(x){
  ...
  if (x=="2024_09_JAX_RNAseq7_Reversion_CBO_day_9_PAX6_DEseq2.tsv"){
    x_d_group = paste(x_split[x_start:(x_end-1)], collapse="_")
    x_gene = x_split[x_end]
    x_strategy = "PTC"
  }else{
    x_d_group = paste(x_split[x_start:(x_end-2)], collapse="_")
    x_gene = x_split[x_end]
    x_strategy = x_split[x_end-1]
  }
  return(c(x_d_group, x_gene, x_strategy))
}
...
colnames(df_file_info) = c("DE_general_group", "gene", "gene_strategy")
df_file_info
...
<span style="color:red">Gene GHRL1 cannot be found in the annotation file. </span>
<span style="color:red">Likely it should be "GRHL1", just misspelled. </span>
<span style="color:red">Manually change it in current plots. </span>

```{r fig.width =9, fig.height=3}

df_file_info$items = rep(NA, nrow(df_file_info))

for (k in 1:nrow(df_file_info)){
  if (df_file_info$gene_strategy[k]=="PTC"){
    df_file_info$items[k] = paste0(df_file_info$DE_general_group[k],
                                   "_", df_file_info$gene[k])
  }else{
    df_file_info$items[k] = paste0(df_file_info$DE_general_group[k],
                                   "_", df_file_info$gene_strategy[k], 
                                   "_", df_file_info$gene[k])    
  }
}
...
## **** note that this is after creating the "item" column
## **** leave the "item" column uncorrected in order to load result files
## **** need to update it after the metadata file is also updated
df_file_info$gene[which(df_file_info$gene=="GHRL1")] = "GRHL1"
...
df = data.frame(KO = df_file_info$items, 
                ...
                strategy = df_file_info$gene_strategy,
                nDE = NA, 
                padj = NA, 
                log2FoldChange = NA)
...
for (k in 1:nrow(df_file_info)){
  ...
  ### *** manually set the gene name back to the misspelled one
  ### *** in order to load results
  if (df$gene_symbol[k]=="GRHL1"){
    fc = paste0("GHRL1", "_", df$strategy[k], "_log2FoldChange")
    padj = paste0("GHRL1", "_", df$strategy[k], "_padj")
  }else{
...
<span style="color:red">Gene GHRL1 cannot be found in the annotation file. </span>
<span style="color:red">Likely it should be "GRHL1", just misspelled. </span>
<span style="color:red">Manually set it back the misspelled one to load result file. </span>
...
for(k in 1:nrow(df_file_info)){
  ...
  res_k = res[,c(1, grep(df_file_info$gene_strategy[k], names(res)))]
  names(res_k) = gsub(paste0(gene_name, "_", df_file_info$gene_strategy[k], "_"), 
                      "", names(res_k))
  ...
  g2 = ggplot(res_k, aes(x=log2FoldChange, y=-log10(padj), 
         ...
         geom_point() + ggtitle(paste0(d_group, "\n", gene_name, "_", df_file_info$gene_strategy[k])) + 
