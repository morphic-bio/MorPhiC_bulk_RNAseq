---
title: "Differential expression analysis for 2024-09 release, JAX_RNAseq5_ExE_CBO"
...
Differential expression using the bulk RNAseq JAX_RNAseq5_ExE_CBO data from the September 2024 release.

There are two model systems involved in this dataset, CBO & PrS.

There are three day values and three run_id values, each run_id corresponding to one day value. 
...
meta = fread("data/September_2024/JAX_RNAseq5_ExE_CBO_meta_data.tsv", 
             data.table = FALSE)
dim(meta)
meta[1:2,]
names(meta)
...
ggplot(meta, aes(x=read_depth, y=q75, color=gene_group, shape = model_system)) +
    geom_point(size=2, alpha=0.7) + ggtitle("Gene counts") + 
  scale_shape_manual(values = c(7, 16)) + 
  xlab("read depth (million)") + ylab("75 percentile of gene expression") + 
...
For the September 2024 release of JAX_RNAseq5_ExE_CBO, there are two model systems: CBO and PrS. 
...
Run DE analysis for each (model system, day) combination separately. 

When the WT samples correspond to different knockout gene group, use each combination of (model system, day, knockout gene group) as the DE group. 

Run DESeq2 for different DE groups separately
    
      CBO, day 8 and day 22 separately
      PrS, day 6, for three knockout gene groups separately. 
...
release_name = "2024_09_JAX_RNAseq5_ExE_CBO"
...
for(model1 in unique_model_ss){
  ...
  cols2kp = c("model_system", "ko_strategy", "ko_gene", "gene_group", "run_id", "run_id_short", 
              "timepoint", "q75")
  ...
  # day 6
  # but need run for different knockout gene groups separately

  sample_info$de_grp = paste0(sample_info$model_system, 
                              "_day_", 
  ...
  for (d_group in sorted_de_grps){
    ...
    for(geno in genos){
      ...
      setting_name = unique(paste0(sample_info_dg$model_system, 
                                   "_day_", 
                                   as.character(sample_info_dg$day)))
