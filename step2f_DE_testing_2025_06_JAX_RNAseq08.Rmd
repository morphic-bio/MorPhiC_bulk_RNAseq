---
title: "Differential expression analysis for 2025-06 release JAX_RNAseq08"
...
path = "../../MorPhiC/data/MorPhiC_internal_releases/MorPhiC_Release_June_2025/JAX_RNAseq08/"
...
meta = fread("data/June_2025/JAX_RNAseq08_meta_data.tsv", data.table = FALSE)
dim(meta)
meta[1:2, ]
names(meta)
...
unique(meta$label)
meta = meta |>
  mutate(condition = sapply(label, \(x) {
    x = strsplit(x, '-')[[1]]
    ifelse(length(x) == 4, x[4], 'nor')
  }))
table(meta$label, meta$condition)
```

### Add gene-condition group information
...
# Add gene-condition level as one factor to specify which samples go to same DE analysis
# Gene group is not separable with condition
# KO samples ASCL2, ELF5 go to multiple gene groups, but one gene_condition group
table(meta$clonal.description, meta$condition)
# List all gene-condition groups
gene_condition_group = table(
  meta[meta$ko_gene == 'WT', 'clonal.description'], 
  meta[meta$ko_gene == 'WT', 'condition']
) |>
  unclass() |>
  as.data.frame() |>
  ...
  mutate(
    ...
    condition = mapply(\(hyp, nor) {
      stopifnot(sum(c(hyp, nor) == 0) == 1)
      ifelse(hyp == 0, 'nor', 'hyp')
    }, hyp, nor),
    gene_condition_group = paste0(gene_group, '_', condition)
  )
gene_condition_group
# Create gene_condition_group column for meta data
meta$gene_condition_group = unlist(plyr::alply(meta, 1, \(x) {
  group = if (x$ko_gene != 'WT') {
    gene_condition_group |>
      filter(
        condition == x$condition, 
        sapply(gene_list, \(genes) x$ko_gene %in% genes)
      ) |>
      pull(gene_condition_group)
  } else {
    gene_condition_group |>
      filter(
        condition == x$condition,
        desc == x$clonal.description
      ) |>
      pull(gene_condition_group)
  }
...
table(meta$ko_gene, meta$gene_condition_group, meta$condition)
```
...
gPC = ggplot(PC_df, aes(x = PC1, y = PC2, shape=gene_condition_group, color=model_system)) +
  geom_point(size=2.5, alpha=0.7) + scale_color_brewer(palette = "Dark2") +
  scale_shape_prism() + 
  labs(color="Model system", shape ="Gene-Condition group") + 
...
gPC = ggplot(PC_df, aes(x = PC1, y = PC2, shape=gene_condition_group,
                        color=run_id_short)) +
  geom_point(size=2.5, alpha=0.7) + 
  scale_color_brewer(palette = "Dark2") +
  scale_shape_prism() + 
  labs(color="Run_id", shape ="Gene-Condition group") + 
  xlab(sprintf("PC1 (%.1f%% variance)", pvs[1])) + 
  ylab(sprintf("PC2 (%.1f%% variance)", pvs[2])) + 
  ggtitle("Wild type samples") + 
  guides(
