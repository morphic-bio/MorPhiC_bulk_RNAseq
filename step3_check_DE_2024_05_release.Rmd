---
title: "Check the results of differential expression analysis for 2024-05 release"
author: "Wei Sun"
date: "`r Sys.Date()`"
output: 
  html_document:
...
## Read in DE results
```{r fig.width = 4, fig.height=3}
PrS_nor = fread("results/2024_05_PrS_DEseq2.tsv", data.table = FALSE)
dim(PrS_nor)
PrS_nor[1:2,1:7]

ExM_nor = fread("results/2024_05_ExM_DEseq2.tsv", data.table = FALSE)
dim(ExM_nor)
ExM_nor[1:2,1:7]

stopifnot(all(PrS_nor$gene_ID == ExM_nor$gene_ID))

res = merge(PrS_nor, ExM_nor)
...
items = unique(str_extract(items, "^[a-zA-Z0-9]+_[a-zA-Z0-9]+"))
items
...
for(it1 in items){
  ...
  res_k$DE = "No"
  res_k$DE[res_k$log2FoldChange > log2(1.5) & res_k$padj < 0.05] <- "Up"
  res_k$DE[res_k$log2FoldChange < -log2(1.5) & res_k$padj < 0.05] <- "Down"
...
fwrite(res, file = "results/2024_05_DEseq2.tsv", append = FALSE, sep = "\t")
system("gzip -f results/2024_05_DEseq2.tsv")
```
