...
extra_cols = setdiff(names(cts)[-1], meta$file_id)
cts = cts[,-which(names(cts) %in% extra_cols)]
...
## DE analysis with respect to model system
...
for(model1 in unique(meta$model_system)){
  ...
  fwrite(res_df, sprintf("results/May_release_%s_DEseq2.tsv", model1), sep="\t")
}
```
...
gc()
sessionInfo()
renv::snapshot(lockfile = "step2b_DE_testing_May_release.lock")
```
