---
title: "Check RNAseq data of 2024-09 release, JAX_RNAseq7_Reversion"
...
Check the bulk RNAseq JAX_RNAseq7_Reversion data from September 2024 release.

This dataset contains samples from reverting gene deletion by removal of a premature termination codon. Need to record the information on whether a non-WT sample is from PTC or the reverted version. 
...
meta_file = "../../MorPhiC/data/September-2024/JAX_RNAseq7_Reversion/metadata/STUDY7_DPC_Metadata_template_v2.xlsx"
...
```{r}
print("These do not include the PTC situations:")
...
for (i in 1:nrow(dc)){
  if (is.na(dc[["ko_strategy"]][i])){
    cur_desc = dc$differentiated_biomaterial_description[i]
    second_substring = str_split(cur_desc, " ")[[1]][2]
    dc[["ko_strategy"]][i] = second_substring
  }
}

table(dc$ko_strategy)
dc$ko_strategy[which(dc$ko_strategy=="derived")] = NA
table(dc$ko_strategy, useNA="ifany")

dc[["ko_gene"]] = str_extract(dc$differentiated_biomaterial_description, 
                                  "(?<=KOLF2.2 deleted |KOLF2.2J reverted ).\\S+")
...
eas_protocol = rep("JAXPR001", nrow(meta))
eas_protocol[meta$ko_strategy=="PTC"] = "JAXPE003"
table(paste0(eas_protocol, "_", meta$biomaterial_id)==meta$protocol_id, 
...
# verify consistency between ko_gene (limited to reverted) got here and 
# those in the tab "Expression alteration strategy"
...
fwrite(meta, file = "data/September_2024/JAX_RNAseq7_Reversion_meta_data.tsv", sep="\t")
```
