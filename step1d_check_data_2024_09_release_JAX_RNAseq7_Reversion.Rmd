...
meta_file = paste0(metadata_path, "/v7_JAX_RNAseq7_Prod.xlsx")
...
ko_gene_eas_matched = meta[match(eas$expression_alteration.label, 
                                 meta$expression_alteration.label), ]$ko_gene
table(ko_gene_eas_matched==eas$expression_alteration.genes.altered_gene_symbol, 
      useNA="ifany")
...
# all differences below are due to partial correction of the GHRL gene name
# it is fixed in the file_id column in meta table
# but not fixed in the count data file column names yet of the time of running the code

setdiff(names(cts)[-1], meta$file_id)
setdiff(meta$file_id, names(cts)[-1])

setdiff(gsub("GHRL", "GRHL", names(cts)[-1]), meta$file_id)
setdiff(meta$file_id, gsub("GHRL", "GRHL", names(cts)[-1]))
```
