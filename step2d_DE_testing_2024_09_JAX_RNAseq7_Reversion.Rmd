---
title: "Differential expression analysis for 2024-09 release JAX_RNAseq7_Reversion"
...
Differential expression using the bulk RNAseq JAX_RNAseq7_Reversion data from the September 2024 release.

This dataset is mostly about reverting the PTC. 
...
For one gene PAX6, there are both knockout samples and reverted samples. 

There are two day values and three run_id values, each run_id corresponding to one day value. 
...
path = "../../MorPhiC/data/September-2024/JAX_RNAseq7_Reversion/"
...
meta = fread("data/September_2024/JAX_RNAseq7_Reversion_meta_data.tsv", 
             data.table = FALSE)
dim(meta)
meta[1:2,]
names(meta)
...
table(meta$ko_gene, meta$run_id, meta$condition, useNA="ifany")
```
...
table(meta$run_id, meta$timepoint_value)
...
extracted_ko_string_list = str_extract_all(meta_m$clonal.description, 
...
for (ww in w_gene_group_NA){
  xx = str_extract_all(meta$clonal.description[ww], 
...
For the September 2024 release of JAX_RNAseq7_Reversion, there are two model systems: CBO and PrS, and two conditions, nor and hyp. 

There are both reverted and PTC (treated as knockout strategy) samples.
...
for(cur_mc in unique_model_condition_ss){
  ...
  # that from filenames
  extracted_ko_strings = str_extract_all(meta_m$file_id, "R_PTC|PAX6_PTC|R_WT|WT")
  ...
  gPC = ggplot(PC_df, aes(x = PC1, y = PC2, shape=ko_gene, color=run_id_short)) +
              ...
              xlab(sprintf("PC1 (%.1f%% variance)", pvs[1])) + 
              ylab(sprintf("PC2 (%.1f%% variance)", pvs[2])) + 
              ggtitle(paste0(cur_mc, " for knockout genes")) + 
              guides(
                color = guide_legend(title = NULL, order = 1),
                shape = guide_legend(title = NULL, order = 2)
              ) +
...
In addition, for PrS_nor, even within the same run id robson-14, the difference between WT samples for different knockout gene groups is big. Run DESeq2 for each knockout gene group separately.

Run DESeq2 for different DE groups separately
    
    CBO_nor, day 9
    PrS_hyp, day 6
    PrS_nor, day 6, gene group
...
release_name = "2024_09_JAX_RNAseq7_Reversion"
...
### Run DE analysis for all genes

Treat reverted as a knockout strategy, on the same level as PTC.
...
for(model1 in unique_model_ss){
  ...
  print(table(sample_info$ko_gene, sample_info$ko_strategy)) 
  ...
  ## define ko_group as the combination of knockout gene and knockout strategy
  ...
  for (d_group in sorted_de_grps){
    ...
    print("-----------------------------------")
    print(paste0("Model system: ", model1))  
    if (sum(sample_info_dg$ko_group=="WT_WT") < 2){
      print(sprintf("DE group %s has only %d wild type samples", 
                    d_group, sum(sample_info_dg$ko_group=="WT_WT")))
    ...
    for(geno in genos){
      ...
      for (k_grp1 in ko_grp){
        ...
        if (k_grp1=="reverted"){
          fwrite(res_df, 
                 sprintf("%s/%s_%s_reverted_%s_DEseq2_raw.tsv", 
                         raw_output_dir, release_name, setting_name, geno), 
                 sep="\t")
          fwrite(res_reduced_df, 
                 sprintf("%s/%s_%s_reverted_%s_DEseq2.tsv", 
      ...
      if (all(ko_grp==c("PTC", "reverted"))){
        ...
        w_rev = get_index(res_geno[["reverted"]])
        ...
        print(paste0("number of DE genes from reverted: ", 
                     as.character(length(w_rev))))
  
        ptc_rev_overlap = length(intersect(rownames(res_geno[["PTC"]])[w_ptc], 
                                           rownames(res_geno[["reverted"]])[w_rev]))
        
        print(paste0("number of common DE genes by knock out strategies PTC and reverted: ", 
                     as.character(ptc_rev_overlap)))
  
        if (max(length(w_ptc), length(w_rev)) > 0){
          m = make_comb_mat(list("PTC" = rownames(res_geno[["PTC"]])[w_ptc], 
                                 "reverted" = rownames(res_geno[["reverted"]])[w_rev]))
          g_up = UpSet(m)
          print(g_up)
        }
...
There are 12 samples involved. 6 WT and 6 EPAS1 reverted samples. Half of them are under normoxia and the other half are under hypoxia. 
...
For the 6 reverted samples, they are from three single cell clones, with each clone having two groups of cells developed under different conditions. 
