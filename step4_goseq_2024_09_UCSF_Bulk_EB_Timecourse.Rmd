...
```
The code applies to 2024_09_UCSF_Bulk_EB_Timecourse dataset. 
...
extract_cores <- function(x, str_before, str_after){
  ...
  x_d_group <- paste(middle_substrings[1:2], 
                     collapse = "_")
  x_comparison <- paste(middle_substrings[3:length(middle_substrings)], 
                        collapse = "_")
  
  return(c(x_d_group, x_comparison))
}
...
colnames(df_file_info) = c("model_type", "comparison")

df_file_info$items = paste(df_file_info$model_type, 
                           df_file_info$comparison, sep="_")
df_file_info$items
df_file_info
...
df_file_info_ordered = df_file_info[match(compare_order, df_file_info$comparison),]
df_file_info_ordered
```
...
for (i in 1:nrow(df_file_info_ordered)){
  
  it1 = df_file_info_ordered$items[i]
  compare_name = df_file_info_ordered$comparison[i]
