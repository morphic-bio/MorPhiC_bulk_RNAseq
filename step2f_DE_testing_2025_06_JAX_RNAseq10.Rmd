---
title: "Differential expression analysis for 2025-06 release JAX_RNAseq10"
...
Differential expression using the bulk RNAseq JAX_RNAseq10 data from the June 2025 release.
...
```{r}
unique(meta$clonal.description)
# Add clonal.description for WT clones, to specify which KO samples they serve as controls for
table(meta$clonal.label, meta$ko_gene)
meta[meta$ko_gene == 'WT', 'clonal.description'] = sapply(meta[meta$ko_gene == 'WT', 'clonal.label'], \(wt_clone_label) {
  ko_clone_labels = sub("-WT\\d+$", "", wt_clone_label)
  ko_clone_labels = strsplit(ko_clone_labels, split = '_', fixed = TRUE)[[1]]
  if (length(ko_clone_labels) > 1) {
    ko_clone_labels = c(
      ko_clone_labels[1], 
      paste0("MOK20", sprintf("%03d", as.integer(ko_clone_labels[-1])))
    )
  }
  rx = paste0("^(", paste(ko_clone_labels, collapse = "|"), ")")
  ko_genes = sort(unique(meta$ko_gene[grepl(rx, meta$clonal.label) & meta$ko_gene != "WT"]))
  stopifnot(all(!is.na(ko_genes)), length(ko_genes) > 0)
  paste0("KOLF2.2J control for ", paste(ko_genes, collapse = ", "))
})
meta[meta$ko_gene == 'WT', c('clonal.label', 'clonal.description')]

# Add gene group as one factor to specify which samples go to same DE analysis
# One gene only goes to one gene_group in this dataset
# List all gene groups
gene_group = table(meta[meta$ko_gene == 'WT', 'clonal.description']) |>
  unclass() |>
  as.data.frame() |>
  (\(x) { 
    names(x) = 'nor'
  ...
  mutate(
    gene_group = sapply(strsplit(desc, 'for '), \(x) paste0(strsplit(x[2], ', ')[[1]], collapse = '_')),
    gene_list = strsplit(gene_group, '_')
  )
gene_group
# Create gene_group column for meta data
meta$gene_group = unlist(plyr::alply(meta, 1, \(x) {
  group = if (x$ko_gene != 'WT') {
    gene_group |>
      filter(sapply(gene_list, \(genes) x$ko_gene %in% genes)) |>
      pull(gene_group)
  } else {
    gene_group |>
      filter(desc == x$clonal.description) |>
      pull(gene_group)
  }
  stopifnot(length(group) == 1)
  group
}))
table(meta$ko_gene, meta$gene_group)
table(meta$ko_gene, meta$gene_group, meta$model_system)
```
...
gene_info = data.frame(
  t(apply(cts_dat, 1, tapply, meta$model_system, min)), 
  t(apply(cts_dat, 1, tapply, meta$model_system, median)), 
  t(apply(cts_dat, 1, tapply, meta$model_system, \(x) quantile(x, probs = 0.75))), 
  t(apply(cts_dat, 1, tapply, meta$model_system, max))
)
gene_info[1:2, ]
names(gene_info) = paste0(
  rep(c("ENDO_", "MESO_"), times = 4), 
...
gPC = ggplot(PC_df, aes(x = PC1, y = PC2, shape=gene_group, color=model_system)) +
  geom_point(size=2.5, alpha=0.7) + scale_color_brewer(palette = "Dark2") +
  scale_shape_prism() + 
  labs(color="Model system", shape ="Gene group") + 
...
gPC = ggplot(PC_df, aes(x = PC1, y = PC2, shape=gene_group,
                        color=run_id_short)) +
  geom_point(size=2.5, alpha=0.7) + 
  scale_color_brewer(palette = "Dark2") +
  scale_shape_prism() + 
  labs(color="Run_id", shape ="Gene group") + 
  xlab(sprintf("PC1 (%.1f%% variance)", pvs[1])) + 
  ylab(sprintf("PC2 (%.1f%% variance)", pvs[2])) + 
  ggtitle("Wild type samples") + 
  guides(
...
for(cur_mc in unique_model_condition_ss){
  ...
  # that from filenames
  extracted_ko_strings = str_extract_all(meta_m$file_id, "PTC|WT|KO")
...
Run DE analysis for each gene group separately. They natually separate all batches. 

```{r}
table(meta$gene_group, meta$run_id, meta$model_system, meta$timepoint)
```
...
for(model1 in unique_model_ss){
  ...
  for (d_group in sorted_de_grps){
    ...
    for(geno in genos){
      ...
      tag1 = sprintf("%s_%s_", geno, strategy)
