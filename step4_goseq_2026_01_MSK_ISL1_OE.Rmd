...
```
This code file can be applied to 2026_01_MSK_bulkRNAseq_ISL1_OE.
...
# regular_DE_files is for ISL1 OE versus control within each background.
# condition_DE_files is for control KO versus WT comparisons.
...
extract_cores <- function(x, str_before, str_after){
  ...
  middle_string <- sub(str_before, "", x, fixed = TRUE)
  middle_string <- sub(str_after, "", middle_string, fixed = TRUE)
  middle_substrings <- strsplit(middle_string, "_")[[1]]
  n_substrings <- length(middle_substrings)
  x_d_group <- paste(middle_substrings[1:(n_substrings-3)], collapse = "_")
  x_cell_line <- middle_substrings[n_substrings-2]
  if (n_substrings > 7){
    x_d_group <- paste(middle_substrings[1:(n_substrings-4)], collapse = "_")
    x_cell_line <- paste(middle_substrings[(n_substrings-3):(n_substrings-2)],
                         collapse = "_")
  }
  x_comparison <- paste(middle_substrings[(n_substrings-1):n_substrings],
                        collapse = "_")
  x_gene <- middle_substrings[n_substrings-1]
  
  return(c(x_d_group, x_cell_line, x_gene, x_comparison))
}
...
df_file_info$file_name = regular_DE_files
df_file_info$items = paste(df_file_info$DE_general_group,
                           df_file_info$cell_line,
                           df_file_info$comparison_name, sep="_")
df_file_info$items
df_file_info


# extract core strings from control KO versus WT DE result files
...
extract_cores_cond <- function(x, str_before, str_after){
  ...
  middle_substrings <- strsplit(middle_string, "_")[[1]]
  n_substrings <- length(middle_substrings)
  x_d_group <- paste(middle_substrings[1:(n_substrings-5)], collapse = "_")
  x_oe_setting <- middle_substrings[n_substrings-4]
  x_gene <- middle_substrings[n_substrings-3]
  x_comparison_name <- paste(middle_substrings[(n_substrings-3):(n_substrings-2)],
                             collapse = "_")
  x_compare <- paste(middle_substrings[(n_substrings-3):n_substrings],
                     collapse = "_")
  
  return(c(x_d_group, x_oe_setting, x_gene, x_comparison_name, x_compare))
}
...
if (length(condition_DE_files)>0){
  ...
  df_file_info_cond$file_name = condition_DE_files
  df_file_info_cond$items = paste(df_file_info_cond$DE_general_group,
                                  df_file_info_cond$OE_setting,
...
for (i in 1:nrow(df_file_info)){
  ...
  compare_name = df_file_info$comparison_name[i]
    
  res = fread(file.path(processed_output_dir, df_file_info$file_name[i]),
              data.table = FALSE)
  res[1:2,]
  
  print(paste0(it1, " ", compare_name))
  
  de_gene_sets = list()
    
  res_k = res[,c(1, grep(paste0("^", compare_name, "_"), names(res)))]
  ...
  de_gene_sets[[paste0(compare_name, "_up")]] = res_k[ww_up,]$gene_ID
  de_gene_sets[[paste0(compare_name, "_down")]] = res_k[ww_down,]$gene_ID
...
```
### Control KO versus WT DE results if available
...
if (length(condition_DE_files)>0){
  ...
  for (i in 1:nrow(df_file_info_cond)){
    ...
    compare_name = df_file_info_cond$comparison_name[i]
    print(compare_name)
    
    res = fread(file.path(processed_output_dir, df_file_info_cond$file_name[i]), 
                data.table = FALSE)
    res[1:2,]
