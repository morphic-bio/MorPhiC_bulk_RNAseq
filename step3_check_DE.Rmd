...
gene_info[gene_info$PrS_min > 5e4, -c("description")]
...
## Read in DE results
```{r fig.width = 4, fig.height=3}
PrS_nor = fread("results/PrS_nor_DEseq2.tsv", data.table = FALSE)
dim(PrS_nor)
PrS_nor[1:2,]

PrS_hyp = fread("results/PrS_hyp_DEseq2.tsv", data.table = FALSE)
dim(PrS_hyp)
PrS_hyp[1:2,]

ExM_nor = fread("results/ExM_nor_DEseq2.tsv", data.table = FALSE)
dim(ExM_nor)
ExM_nor[1:2,]
...
ggplot(df, aes(x=CE_log2FoldChange, y=-log10(CE_padj), col=Model, shape=Condition, 
               label=gene_symbol)) + 
    geom_point() +   scale_shape_manual(values = c(7, 16)) + 
...
ggplot(df, aes(x=KO_log2FoldChange, y=-log10(KO_padj), col=Model, shape=Condition, 
               label=gene_symbol)) + 
    geom_point() +   scale_shape_manual(values = c(7, 16)) + 
...
targets = which(res$PrS_nor_FOSB_KO_padj > 0.01 & res$PrS_nor_FOSB_PTC_padj < 1e-10)
length(targets)
res$gene_ID[targets]
